...
- Source: `erlef/otp_builds` の `OTP-<ver>` リリース、`otp-<triple>.tar.gz`
- 対応: **macOS のみ**（Apple Silicon + Intel）。upstream に Linux/Windows
  prebuilt が無いため、それ以外は明示メッセージで bail。
- 検証: `<asset>.sigstore` の DSSE provenance に attest された sha256。
  Sigstore 署名チェーン（Fulcio/Rekor）のフル検証は v1.0 ロードマップ送り。
...
- Erlang の Linux/Windows 対応（upstream prebuilt 待ち）。
