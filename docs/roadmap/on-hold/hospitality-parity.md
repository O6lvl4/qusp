...
### P. Exit code conventions

| ID | 項目 | uv | qusp | 判定 |
|---|---|---|---|---|
| P1 | success | 0 | 0 | ✅ |
| P2 | unknown subcommand | 2 | 2 | ✅ |
| P3 | install impossible version | 2 | **1** | 🟡 conv 違い (1=generic err, 2=usage err — qusp は混在) |
| P4 | missing required arg | 0 (uv は no-arg で latest を install する semantics) | 1 | scope 違い |

### Q. `--help` quality / discoverability

| ID | 項目 | uv | qusp | 判定 |
|---|---|---|---|---|
| Q1 | subcommand help の richness | 多段 description + alternative names + 各 flag の `[env: VAR=]` 表示 + 例示 | 5 行 minimal "Usage: qusp run [OPTIONS] [ARGV]..." のみ | ❌ 大幅劣勢 |
| Q2 | help paginate (`uv help <cmd>` で less に流す) | `--no-pager` で opt-out 可 | 無し (常に flat 出力) | 🟡 Partial |

### R. JSON / machine-readable output

| ID | 項目 | uv | qusp | 判定 |
|---|---|---|---|---|
| R1 | `--output-format json` | あり (`uv python list --output-format json` で full structured: version_parts, implementation, arch, libc, url) | flag 無し (`--json` に "unexpected argument" error) | ❌ Missing |

### S. Environment variable contracts

| ID | 項目 | uv | qusp | 判定 |
|---|---|---|---|---|
| S1 | `--help` に env var 併記 | `--cache-dir [env: UV_CACHE_DIR=]`, `UV_MANAGED_PYTHON`, ... ─ 全 path/flag で env var 露出 | 露出無し (qusp は HOME/XDG_DATA_HOME を読むが、`QUSP_*` 形式の env contract は doc にしか無い) | ❌ Missing |

### T. Multi-vendor variants in list (qusp 専有強み)

| ID | 項目 | uv | qusp | 判定 |
|---|---|---|---|---|
| T1 | LTS / variant marker | N/A (Python 1 vendor) | `21.0.11+12 (LTS)` 風に LTS marker 表示 | qusp 専有 ✓ |

### U. Update detection (outdated)

| ID | 項目 | uv | qusp | 判定 |
|---|---|---|---|---|
| U1 | newer version detection | `uv tool upgrade [--all]`、project deps は `uv lock --upgrade` | `qusp outdated` (toolchain 単位) | ✅ Parity (scope 違いだが両方持ってる) |

### V. Argv passthrough

| ID | 項目 | uv | qusp | 判定 |
|---|---|---|---|---|
| V1 | script に追加 args を渡す | `uv run script.py --verbose --foo` ─ argv[1..] が script へ | `qusp x ./script.lua --verbose --foo` ─ arg[1..] が script へ | ✅ Parity (言語別の argv[0] 慣習差はあり) |

### W. Concurrent invocation safety

| ID | 項目 | uv | qusp | 判定 |
|---|---|---|---|---|
| W1 | 並列 install の race 防止 | `Acquired lock for /tmp/.../python` (file lock 機構)、release も明示 log | **コード grep で `flock` / `fs2::` / `FileLock` 0 hit** ─ file lock 機構無し、2 並列 `qusp install` で race 条件 | ❌ qusp バグ (Phase 5 外、F4 と同じく correctness) |

### Y. Verbose / `-v` flag richness

| ID | 項目 | uv | qusp | 判定 |
|---|---|---|---|---|
| Y1 | `-v` で DEBUG-level 出力 | "Acquired lock for X / Found download Y / Downloading URL / Extracting to tmp / Moving X to Y / Installed at X / Released lock" 等の構造化 trace | `--verbose` flag 無し (`--quiet` のみ) | ❌ Missing |

### Z. Help paginate / docs ergonomics

| ID | 項目 | uv | qusp | 判定 |
|---|---|---|---|---|
| Z1 | `<cmd> help <subcmd>` で long help を pager 経由 | あり、`--no-pager` で opt-out | 無し | 🟡 (Q2 と同根) |

---

## Verdict 集計 (40 項目, P-Z 拡張後)

| 状態 | 件数 | 該当 row |
|---|---|---|
| ✅ Parity (or qusp 勝ち) | **15 件** | A1 / A2 / A6 / C1 / C3 / D5 / E2 / F3 / G1 / J1 / M1 / O1 / P1 / P2 / U1 / V1 |
| 🟡 Partial (動くが劣る) | **13 件** | A4 / A5 / A5b / B1 / B3 / C2 / D1 / D6 / E1 / I1 / K2 / P3 / Q2 / Z1 |
| ❌ Missing (不在 or 大幅劣る) | **13 件** | A3 / A7 / B4 / D2 / D3 / D4 / G2 / G3 / K1 / N1 / Q1 / R1 / S1 / Y1 |
| バグ (Phase 5 外、correctness) | **2 件** | F4 / W1 |
| qusp 専有 (✅ に含む) | A6 / D5 / H1 / T1 | |
| scope 違い (比較不能) | **3 件** | F1 / F2 / P4 |
| 設計 trade-off (deliberate) | **4 件** | B2 / I2 / I3 / L1 |
...
- ❌ **[`--help` richness + pager](help-richness-and-pager.md)**
  ─ 解決対象: **Q1** + **Q2** + **Z1**。subcommand help を多段
    description + alternatives + 例示 + env var 表記まで埋める。
    long help は pager 経由 (`--no-pager` で opt-out)。

- ❌ **[Machine-readable JSON output](machine-readable-output.md)**
  ─ 解決対象: **R1**。`qusp <cmd> --output-format json` で
    list / current / outdated / doctor を structured JSON に。
    CI / scripting 用途で `jq` パイプ可能に。

- ❌ **[Verbose / `-v` debug log](verbose-flag.md)**
  ─ 解決対象: **Y1** + **S1** (env var 露出の docs 側補完)。
    `-v` で download / extract / lock acquisition / publisher URL を
    DEBUG-level trace、env contract も `--help` に併記。
...
> + 上記 audit 表を再走したとき:
>
> 評価対象 = 40 - 3 (scope 違い: F1/F2/P4) - 4 (trade-off: B2/I2/I3/L1) - 2 (バグ: F4/W1) = **31 項目**
>
> - ✅ 列が **31 件中 24 件以上 (≥ 77%)**
> - ❌ 列が **0 件**
> - 全 🟡 項目に明示的な reasoning (なぜ ✅ にならないか) が doc に残ってる

現状 (v0.24.0): ✅ 15 / 🟡 13 / ❌ 13 (= 41 評価対象、~37% ✅)
完了時 (Phase 5 終): ✅ 24+ / 🟡 7 以下 / ❌ 0 (≥ 77% ✅)
...
audit 中で **hospitality ではなく correctness バグ** として浮上した 2 件:

- **F4: failed install で qusp.lock が作られない** ─ partial-success
  シナリオで lock が空のままになり、`sync --frozen` で復旧できない。
  A7 (network resilience) の延長線で 1 つの fix path に。
- **W1: concurrent invocation の race condition** ─ qusp は file lock を
  持たないので、2 並列 `qusp install` で content-addressed store の
  race 条件 / partial extract / sha mismatch crash の可能性。詳細は
  [concurrent-install-safety.md](concurrent-install-safety.md)。

両方とも Phase 5 外で個別 issue / fix。Phase 5 hospitality と並列で進める。
